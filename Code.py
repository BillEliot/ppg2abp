...
if __name__ == '__main__':
    ...
    for sliding_window_width in [62, 125, 187, 250, 312, 375]:
        ...
        for fold in range(10):
            fold_train_PPG = np.concatenate((train_PPG[ : int(len(train_PPG) * fold * 0.1)], train_PPG[int(len(train_PPG) * (fold + 1) * 0.1) : ]))
            fold_train_ABP = np.concatenate((train_ABP[ : int(len(train_ABP) * fold * 0.1)], train_ABP[int(len(train_ABP) * (fold + 1) * 0.1) : ]))
            fold_test_PPG = train_PPG[int(len(train_PPG) * fold * 0.1) : int(len(train_PPG) * (fold + 1) * 0.1)]
            fold_test_ABP = train_ABP[int(len(train_ABP) * fold * 0.1) : int(len(train_ABP) * (fold + 1) * 0.1)]
            ...
            if rmse < rmse_last:
                rmse_last = rmse
                model.save('./model_%d.h5' % sliding_window_width)
